  ...
  document.querySelector('#root').innerHTML = `
    <div class="app-shell">
      <aside class="sidebar ${state.sidebar ? 'sidebar-open' : ''}">
        ...
        <div class="local-note"><span class="status-dot ${state.error ? 'offline' : ''}"></span><span>${state.error ? '本地存储不可用' : '数据域独立存储'}</span>${!state.error ? '<button data-action="open-data">记忆备份</button>' : ''}</div>
      </aside>
...
function dataModal() {
  return `<div class="modal-backdrop"><section class="modal data-modal" role="dialog" aria-modal="true" aria-labelledby="data-title"><header><div><p class="eyebrow">LOCAL DATA</p><h2 id="data-title">数据与备份</h2></div><button class="icon-button" data-action="close-data" aria-label="关闭">${icon('x')}</button></header><div class="data-modal-body"><div class="storage-summary">${icon('database',24)}<div><strong>${state.items.length} 段记忆保存在此浏览器</strong><span>${state.storagePersistent ? '浏览器已授予持久存储权限' : '建议定期导出 JSON，防止清理浏览器数据时丢失'}</span></div></div><div class="data-actions"><button data-action="export-data">${icon('download',20)}<span><strong>导出 JSON</strong><small>下载全部内容和复习进度</small></span></button><button data-import="merge">${icon('upload',20)}<span><strong>合并导入</strong><small>保留现有数据，相同 ID 以备份为准</small></span></button><button data-import="replace">${icon('archive',20)}<span><strong>覆盖导入</strong><small>清空现有内容后恢复备份</small></span></button></div><input id="backup-file" type="file" accept="application/json,.json" hidden><div class="privacy-note"><strong>隐私说明</strong><p>数据只存在当前浏览器中，阿里云服务器和 Nginx 不会接收你的背诵内容。更换浏览器、域名或端口前，请先导出备份。</p></div><footer><button class="danger-text-button" data-action="clear-data">清空此浏览器的全部数据</button><button class="secondary-button" data-action="close-data">完成</button></footer></div></section></div>`;
}
...
function exportBackup() {
  const backup = { format: 'shiyi-memory-backup', version: 1, exportedAt: new Date().toISOString(), items: state.items };
  const blob = new Blob([JSON.stringify(backup, null, 2)], { type: 'application/json' });
  const url = URL.createObjectURL(blob);
  const link = document.createElement('a');
  link.href = url;
  link.download = `shiyi-backup-${today}.json`;
  link.click();
...
document.addEventListener('click', async event => {
  ...
  if(action==='export-data'){exportBackup();notify('JSON 备份已导出');return}
  if(action==='clear-data'){if(!window.confirm('确定清空此浏览器中的全部背诵内容和复习记录吗？此操作无法撤销。'))return;await browserStore.clear();state.items=[];state.dataModal=false;notify('浏览器数据已清空');return}
  ...
  const importer=event.target.closest('[data-import]'); if(importer){state.importMode=importer.dataset.import;if(state.importMode==='replace'&&!window.confirm('覆盖导入会先清空当前全部数据，确定继续选择备份文件吗？'))return;document.querySelector('#backup-file')?.click();return}
